...
const createBrotherService = ({
  ...
  const printJob = async ({
    ...
    try {
      ...
      // Leading/trailing feed margin in dots. 14 dots (~2mm at 180dpi) is the
      // documented minimum that keeps the printer in pre-cut mode; going below
      // it can drop the printer into "no pre-cut" mode whose minimum margin is
      // ~24mm (the full head-to-cutter clearance), wasting tape before each
      // label. Configurable per printer via brotherMargin.
      const marginDots = Number.isFinite(Number(printerConfig.brotherMargin))
        ? Math.max(0, Math.round(Number(printerConfig.brotherMargin)))
        : 14;

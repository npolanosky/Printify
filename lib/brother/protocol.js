...
// 0x1A), not here.
const buildPrintInfo = ({ mediaType = 0, mediaWidthMm = 0, mediaLengthMm = 0, rasterLines = 0, startingPage = true, validFlags = 0xCE }) => {
  // Valid-flag bits: PI_KIND(0x02) | PI_WIDTH(0x04) | PI_LENGTH(0x08) |
  // PI_QUALITY(0x40) | PI_RECOVER(0x80). Default 0xCE matches the proven
  // single-label job; 0x84 (WIDTH + RECOVER only, like the brother_pt
  // reference) avoids marking media-length valid-but-zero, which can make
  // multi-page jobs render the label as zero-length (content overflow).

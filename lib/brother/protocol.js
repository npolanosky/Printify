...
// 0x1A), not here.
const buildPrintInfo = ({ mediaType = 0, mediaWidthMm = 0, mediaLengthMm = 0, rasterLines = 0, startingPage = true, validFlags = 0x84 }) => {
  // Valid-flag bits: PI_KIND(0x02) | PI_WIDTH(0x04) | PI_LENGTH(0x08) |
  // PI_QUALITY(0x40) | PI_RECOVER(0x80). Default 0x84 = WIDTH + RECOVER only,
  // matching the raster command reference's own worked example and the
  // brother_pt reference. The previous 0xCE also marked KIND and LENGTH valid
  // (with a zero length); the PT-E550W tolerated it but stricter models (e.g.
  // PT-P700) render the label blank, so 0x84 is the safe, documented value.
...
// M selects the raster compression mode. 0x00 = no compression (raw bytes in
// each G line), 0x02 = TIFF/packbits. We send raw data, so select 0x00
// explicitly — without it, a printer defaulting to TIFF mode interprets our
// raw raster as packbits and prints garbage/blank. Selecting no-compression
// also means the Z (zero-line) command must NOT be used: per the raster
// command reference, Z is only valid in TIFF mode.
const buildCompressionMode = (mode = 0x00) => Buffer.from([0x4D, mode]);

...
const createPrintingService = ({
  ...
  const runBrotherPrint = (filePath, printerConfig, done, jobMeta = {}) => {
    ...
    // When the brother branch batches a serial set, jobMeta.brotherChainFiles
    // holds the REMAINING page paths after the first. Combine them with the
    // primary filePath so the service receives the full ordered page list.
    const chainFiles = Array.isArray(jobMeta.brotherChainFiles) && jobMeta.brotherChainFiles.length
      ? [filePath, ...jobMeta.brotherChainFiles]
      : undefined;
  ...
  const printImageItems = async ({ items, printerConfig, printCopies }) => {
    ...
    // service, which prints all copies as one multi-page job (invalidate +
    // init once, each copy a page) per the raster command reference, so the
    // head-to-cutter feed only happens once instead of before every label.
    if (printerConfig.printMode === 'brother') {
      // Multiple DISTINCT files (e.g. a serial-numbered label set): convert each
      // in order, then hand the whole ordered list to the brother service as one
      // batched job. The first converted path is the normal filePath; the rest
      // ride along via brotherChainFiles so the service chains all N pages.
      if (items.length > 1) {
        const outputPaths = [];
        for (const item of items) {
          // Sequential conversion preserves page order for the chained job.
          // eslint-disable-next-line no-await-in-loop
          const outputPath = await converter.convertImgToPdf(
            item.file.path, printerConfig, undefined, item.jobMeta
          );
          outputPaths.push(outputPath);
        }

        const firstItem = items[0];
        const [firstOutputPath, ...restOutputPaths] = outputPaths;

        await printPDF(firstOutputPath, printerConfig, {
          ...firstItem.jobMeta,
          chksum: firstItem.checksum || firstItem.jobMeta.chksum || null,
          pages: items.length,
          totalCopies: null,
          brotherChainFiles: restOutputPaths,
        });
        return items.length;
      }

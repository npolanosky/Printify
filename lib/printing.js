...
const createPrintingService = ({
  ...
  const printImageItems = async ({ items, printerConfig, printCopies }) => {
    ...
    // corrupt the job.  Instead, convert once and send all copies in one USB
    // session via chain printing (0x0C between labels, 0x1A on the last).
    // This also eliminates the head-to-cutter waste margin between copies
    // since the printer only performs the initial tape advance once per session.

...
const createPlugin = ({
  ...
  // skips the actual print dispatch (see printing.js); it must not report a
  // printer as online when nothing is connected.
  const getEffectiveState = nextState => ({
    ...nextState,
    online: Boolean(nextState?.online),
  });
  ...
  const applyObservedState = nextState => {
    currentTargetPrinterIds.forEach(printerId => {
      ...
      // Persist the physical head width so the label builder can size the
      // canvas to the printable area rather than the raw tape width.
      if (Number.isFinite(nextState.maxPrintWidthPx)) {
        serverSave?.setPrinterPreference?.(printerId, 'headWidthPx', nextState.maxPrintWidthPx);
      }
    });
  };
